...
class LoginWindow(tk.Toplevel):
    ...
    def _registar(self):
        ...
        if not self._primeiro:
            dlg = _DialogoAdminAuth(self)
            if not dlg.ok:
                self._r_erro.config(text="Sem autorização de administrador."); return
        ...
        try:
            ...
            if self._primeiro:
                ...
                self._r_role.config(state="normal")
            self._aba("login")
...
class _DialogoAdminAuth(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.ok = False
        self.title("Autorização de Administrador")
