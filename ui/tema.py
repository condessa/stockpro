...
class Tooltip:
    """Tooltip simples que aparece ao passar o rato."""
    def __init__(self, widget, texto):
        ...
        widget.bind("<Destroy>", lambda _: self._esconder())

    def _mostrar(self, _=None):
        if self._janela: return
        self._janela = tw = tk.Toplevel(self._widget)
        tw.overrideredirect(True)
        tw.attributes("-topmost", True)
        tk.Label(tw, text=self._texto,
                 bg=C["bg4"], fg=C["text"],
                 font=("Segoe UI", 8),
                 padx=8, pady=4,
                 highlightthickness=1,
        ...
        tw.update_idletasks()
        x = self._widget.winfo_rootx() + self._widget.winfo_width() // 2 - tw.winfo_width() // 2
        y = self._widget.winfo_rooty() - tw.winfo_height() - 4
        tw.geometry(f"+{x}+{y}")
